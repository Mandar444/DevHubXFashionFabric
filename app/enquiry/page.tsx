  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full flex flex-col items-center relative animate-fade-in">
        ...
        <button
          className="mt-2 px-6 py-2 bg-green-700 hover:bg-green-800 text-white rounded-lg font-medium transition-colors"
          onClick={onClose}
        >
          Close
        </button>
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: result.error || "There was an error submitting your enquiry. Please try again.",
          variant: "destructive",
        })
      }
    } catch (error) {
                  ...
                  <div className="space-y-6">
                    <div className="flex items-start">
                      ...
                      <p className="text-neutral-600">
                        Shop No. 8, Block - II, Dukle Heaven, Near Yamaha Showroom, Santa Inez, Panaji, Taleigao, Goa
                        403001
                      </p>
                    </div>
